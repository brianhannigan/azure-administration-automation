...
class AzureVmManagerApp:
    ...
    def _build_config_tab(self) -> None:
        ...
        ttk.Button(editor_bar, text="Decrypt/Open", command=self.open_selected_config).pack(side="right")
    ...
    def start_selected_vm(self) -> None:
        ...
        if not vm:
            ...
            return
        if not messagebox.askyesno("Confirm Start", f"Start VM '{vm['vm_name']}'?"):
            return
    ...
    def stop_selected_vm(self) -> None:
        ...
        if not vm:
            messagebox.showwarning("No Selection", "Please select a VM first.")
            return
        if not messagebox.askyesno("Confirm Stop", f"Stop VM '{vm['vm_name']}'?"):
            return
    ...
    def choose_config_directory(self) -> None:
        chosen = filedialog.askdirectory(title="Choose Config Folder", initialdir=str(self.config_dir))
        if not chosen:
            return
    ...
    def open_selected_config(self) -> None:
        file_path = self._get_selected_config_path()
        if not file_path:
            messagebox.showwarning("No Selection", "Please select a config file first.")
            return
        try:
            if file_path.suffix.lower() == ".encjson":
